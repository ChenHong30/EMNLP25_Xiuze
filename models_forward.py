...
def gpt2_model_forward(
    ...
    inputs_embeds, u_i_enbeds, img_embeds = inputs_embeds
    ...
    for i, (block, layer_past) in enumerate(zip(self.h, past_key_values)):
        ...
        if self.gradient_checkpointing and self.training:
            outputs = self._gradient_checkpointing_func(
                block.__call__,
                [hidden_states, u_i_enbeds, img_embeds],
                None,
                attention_mask,
                head_mask[i],
...
def gpt2_attention_forward(
    ...
    hidden_states, u_i_enbeds, img_embeds = hidden_states
    if encoder_hidden_states is not None:
        ...
        if lora_flag:
            key, value = self.c_attn([encoder_hidden_states, u_i_enbeds, img_embeds]).split(self.split_size, dim=2)
        else:
            key, value = self.c_attn(encoder_hidden_states).split(self.split_size, dim=2)
        attention_mask = encoder_attention_mask
    else:
        if lora_flag:
            query, key, value = self.c_attn([hidden_states, u_i_enbeds, img_embeds]).split(self.split_size, dim=2)
        else:

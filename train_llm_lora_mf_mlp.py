...
from llm_lora_mf_mlp import ContinuousPromptLearning
...
parser.add_argument('-index_dir', '--index_dir', type=str, default="./data/TripAdvisor/1/",
                    help='load indexes')
parser.add_argument('-llm_model', '--llm_model', type=str, default="./llm/llama-2",
                    help='llm model')
...
parser.add_argument('-post_att', '--post_att', action='store_true',
                    help='post attention')
parser.add_argument('-lora_nums', '--lora_nums', type=str, default="30,31",
...
pad = '<pad>'
unk = '<unk>'
tokenizer = AutoTokenizer.from_pretrained(args.llm_model, bos_token=bos, eos_token=eos, pad_token=pad,
                                          unk_token=unk)
...
model = ContinuousPromptLearning.from_pretrained(args.llm_model, args.pre_model, args.post_att, nuser, nitem, ntoken,
...
def train(data):
    ...
    while True:
        ...
        if data.step % args.log_interval == 0 or data.step == data.total_step:
            ...
            print(
                now_time() + 'text ppl {:4.4f} | rating loss {:4.4f} | {:5d}/{:5d} batches'.format(

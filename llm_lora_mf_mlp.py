...
transformers.models.llama.modeling_llama.LlamaAttention.forward = models_forward.llama_attention_forward
transformers.models.llama.modeling_llama.LlamaDecoderLayer.forward = models_forward.llama_decoder_layer_forward
transformers.models.llama.modeling_llama.LlamaModel.forward = models_forward.llama_model_forward
...
class UIPrompt:
    @classmethod
    def from_pretrained(cls, pretrained_model_name_or_path, pre_model, post_att, nuser, nitem, ntoken, lora_nums,
                        lora_dim, num_heads, pad_token_id, **kwargs):
        ...
        lora_layer = ["model.layers.{}".format(ll) for ll in lora_layer_nums]
        for name, module in model.named_modules():
            lora_layer_bool = sum([lora_layer_name in name for lora_layer_name in lora_layer])
            # [mlp.gate_proj, mlp.up_proj, mlp.down_proj]
            # [self_attn.q_proj, self_attn.k_proj, self_attn.v_proj, self_attn.o_proj]
            if lora_layer_bool > 0 and "self_attn.o_proj" in name:
        ...
        model.init_prompt(pre_model, post_att, lora_layer_nums, nuser, nitem, num_heads, pad_token_id)
        return model

    def init_prompt(self, pre_model, post_att, lora_nums, nuser, nitem, num_heads, pad_token_id):
        self.src_len = 2
        self.post_att = post_att
        self.lora_nums = lora_nums
        self.pad_token_id = pad_token_id

        emsize = self.model.embed_tokens.weight.size(1)  # 4096
    ...
    def forward(self, user, item, text, mask, rating_prediction=True, ignore_index=-100):
        ...
        w_src = self.model.embed_tokens(text)
